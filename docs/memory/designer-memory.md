...
- 2026-04-05 run 61: Focus session mode (MTB-30) — full-screen dark takeover (#1A1A1A) for distraction-free timer; 3-panel composite: entry CTA on campaign detail + running timer screen + session complete screen; large 76px timer display; session progress bar with timestamps; editable target duration pill; "End Session Early" ghost button; auto-log confirmation card (bolt icon + blue border); before→after progress comparison bar on completion; dark screens use rgba borders/shadows (inverted from light design system)
- 2026-04-05 run 64: Account sign-in & registration (MTB-31) — pre-auth screens have no bottom nav or app bar; full-screen with logo block (72×72 icon box + app name + tagline); inline compact logo (44×44) on registration to make room for 3 fields; error state = red banner + red-bordered fields + inline field errors + red CTA ("Try Again"); password strength 4-segment bar on registration; blue info card (lock/cloud icon) at bottom of each screen for security/persistence messaging
- 2026-04-05 run 65: User profile screen (MTB-32) — pushed screen (back arrow, no bottom nav); avatar is 88×88 blue square with initials + persistent photo_camera badge (28×28) in bottom-right corner as upload affordance; device sign-in rows follow same icon-box + meta pattern as stats/info cards; "This device" badge = blue fill (matches active status badge convention); avatar upload state uses semi-transparent blue overlay on avatar + 3-option picker strip (Gallery/Camera/Initials)
- 2026-04-05 run 67: Real-time multi-device sync (MTB-34) — sync status badge in app bar (3 states: synced/syncing/offline); device attribution chips on all activity items; "Your Devices" panel pattern with device type icon + status chip; amber warning bar for offline state; blue catch-up progress bar on reconnection; silent notification card (non-modal, below app bar)
- 2026-04-05 run 68: Account/data management (MTB-35) — pushed screen with 3 sections; color-coded severity: blue=safe (sign out), terracotta=medium (clear local), red=destructive (delete account); danger zone section uses red left-border accent (not blue); destructive rows get red border + rgba(184,50,50) shadow; confirmation dialogs: blue icon = reversible action (reassure with cloud_done), red icon = irreversible action (3-row consequence checklist with close icons); "Keep My Account" / "Delete Forever" footer pattern for highest-severity dialogs
- 2026-04-05 run 66: Cloud sync upload (MTB-33) — pre-app screen (no bottom nav, no app bar); blue security info card on ALL states (syncing/success/error) is essential UX pattern for cloud features — reassures user local data is never overwritten; error state uses red (#B83232) border/shadow on error card (parallels red used in form validation); partial-upload progress bar shown on error state so user knows how much work remains on retry; "Retry Sync" (blue primary) + "Continue Offline" (white secondary) button pair on error
...
| 2026-04-05 run 63 | MTB-5 | Trashed issue — still only issue in "In Design"; skipped (5th skip across runs 56–58, 62, 63); no unhandled issues | Idle run |
| 2026-04-05 run 64 | MTB-31 | Account registration & sign-in — 3-state composite: Sign In (clean), Create Account, Sign In (error). Pre-auth full-screen, no bottom nav. Red banner + red-bordered fields for errors. Compact inline logo on register screen. Password strength bar. Blue security/persistence info card on each screen. | Awaiting Design Approval |
| 2026-04-05 run 65 | MTB-32 | User profile screen — 3-state composite: (1) View Profile: 88×88 avatar square with initials + photo_camera upload badge, display name + email, account info rows (email, member since), last signed-in device list with device type icon + OS/date meta + "This device" (blue) or time-ago badge; (2) Edit Display Name: inline active blue-border input with Cancel+Save row; (3) Avatar Upload: camera overlay on avatar (blue tint + camera icon) + Gallery/Camera/Initials picker strip + black success toast | Awaiting Design Approval |
...
| 2026-04-05 run 75 | (none) | No "In Design" issues found — queue empty | Idle run |
